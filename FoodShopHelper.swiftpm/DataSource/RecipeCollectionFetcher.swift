...
class RecipeCollectionFetcher: ObservableObject {
    @Published var recipeData = RecipeCollection(result: [Recipe.defaultRecipe])
    @Published var currentRecipe = Recipe.defaultRecipe
    
    let urlString = "https://app.rakuten.co.jp/services/api/Recipe/CategoryRanking/20170426?applicationId=1055163922761930703"
    ...
    enum FetchError: Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .badJSON(let error):
                return "Decoding JSON Error! : \(error)"
            }
        }
    }
    ...
    throws {
        ...
        try await MainActor.run {
            do {
                recipeData = try JSONDecoder().decode(RecipeCollection.self, from: data)
            } catch {
...
extension RecipeCollectionFetcher: @unchecked Sendable {}
